...
test("Declaration signing surface uses the authoritative reference texture", () => {
  const source = readFileSync(new URL("./renderers/declaration-signing.ts", import.meta.url), "utf8");
  const provenance = JSON.parse(readFileSync(new URL("../public/historical/declaration-provenance.json", import.meta.url), "utf8"));
  assert.match(source, /reference\.src = "\/historical\/declaration\.jpg"/);
  assert.doesNotMatch(source, /\/reference\/declaration\.jpg/);
